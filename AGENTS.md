...
This repository is the **Trading Academy CRM**: an internal operations platform
for a premium trading academy. Company base: Dubai, UAE. Company timezone:
`Asia/Dubai`. Design language: **Dubai Financial Command Center**.
...
| --- | --- |
| Anything — what the product is, its modules, what is out of scope | `docs/product/PRODUCT_ARCHITECTURE.md` |
| How the product spec maps onto this codebase, what is built, the build order | `docs/product/STACK_MAPPING.md` |
| Clients, leads, students, ownership, conversion, deposits, programs — the schema | `docs/product/DATABASE.md` |
| Who can see or change which client — roles and visibility | `docs/product/PERMISSIONS.md`, `docs/product/USER_ROLES.md`, `docs/product/AUTHORIZATION.md` |
| The route tree and screen structure | `docs/product/ROUTES.md` |
| The market globe | `docs/product/3D_ARCHITECTURE.md` |
| The company calendar | `docs/product/CALENDAR.md` |
| Tests, the permission suite, CI gates | `docs/product/TEST_PLAN.md` |
...
| UI in `apps/app` or `packages/ui` | `docs/design.md` (below) and `docs/product/DESIGN_SYSTEM.md` |
...
Also check `.agents/skills/` for a relevant skill before starting. Product
skills: `trading-academy-design`, `crm-ux`, `dashboard-ux`, `calendar-ux`,
`globe-design`, `3d-design`, `database-architecture`, `security`, `qa`,
`performance`. Engineering skills: better-auth, prisma, nestjs-trpc, eve,
shadcn, nuqs and others. Agents live in `.claude/agents/`: `visual-designer`,
`frontend-engineer`, `3d-engineer`, `database-architect`, `security-reviewer`,
`qa-engineer`. Tell the user which rules and skills you read.
...
- **A client is visible to exactly three people:** an admin, its sales owner,
  its mentor. Enforce it on the server, in one access module, on every read and
  write. A check in a React component hides a control. It protects nothing.
- **Every timestamp is timezone-aware.** Display uses IANA IDs (`Asia/Dubai`,
  `Europe/London`). Never hardcode a UTC offset.
- **Deposits are an append-only ledger.** Totals are derived. Never store a
  mutable total column as the source of truth.
- **Out of scope for v1:** communication center, trading support, trade
  execution, broker integration, trading signals, advanced analytics, social
  features, AI features beyond what the base already ships. Do not build them,
  scaffold them, or prepare for them. Ask first.
- **Never claim a feature is complete** if only the UI exists, the backend is
  mocked, authorization is missing, or the tests have not run. Say what is
  built and what is not.
- **Browser QA is part of the work.** For significant UI, run the app, inspect
  it at desktop, tablet and mobile widths, and test empty, loading and error
  states. A first pass that compiles is not done.
- **Nothing about a person is guessed.** A blank field beats a plausible value.
...
Commit subjects and pull request titles are Conventional Commits:
`feat(app): add the deposits ledger`, `fix(api): scope client reads to the
owner`. The title becomes the changelog line. See `CONTRIBUTING.md`.
