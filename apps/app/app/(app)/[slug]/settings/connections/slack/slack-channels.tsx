...
const INVITE_COMMAND = "/invite @Trading Academy CRM";
			...
			onSuccess: async (result) => {
				...
				toast.success(
					result.alreadyJoined
						? "Trading Academy CRM is already in there."
						: result.queued
							? "Trading Academy CRM is joining."
							: "Ask someone inside to invite Trading Academy CRM.",
				);
			},
					...
					<p className="px-4 py-4 text-muted-foreground text-sm">
						{channels.pending
							...
							: query
								...
								: "No channels yet. Trading Academy CRM reads the list from Slack after it connects."}
					</p>
				}
		...
		<AlertDialog open onOpenChange={(open) => !open && onCancel()}>
			<AlertDialogContent>
				<AlertDialogHeader>
					<AlertDialogTitle>
						{canInviteItself
							? `Add Trading Academy CRM to #${channel.name}?`
							: "Ask someone to add Trading Academy CRM"}
					</AlertDialogTitle>
					<AlertDialogDescription>
						{canInviteItself
							? `It is a private channel, so Trading Academy CRM joins as you. Same as typing the invite yourself. Everyone in the channel sees it join. It reads nothing until you turn a permission on.`
							: `We cannot add Trading Academy CRM to a private channel yet. Someone already in #${channel.name} has to run this.`}
					</AlertDialogDescription>
				</AlertDialogHeader>
