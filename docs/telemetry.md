...
**Where it goes** is your own PostHog project. Set `CRM_TELEMETRY_KEY` (a write-only `phc_`
key) and, for a self-hosted or EU instance, `CRM_TELEMETRY_HOST`. With no key set no client is
created and nothing is sent. Nothing is sent while `NODE_ENV` is `test`, so a test run cannot emit.
...
- **Server side only, in everything you run.** `posthog-node` in `apps/api` and `apps/agent`.
  ...
  amounts out of your database; the simplest way not to hold that is not to collect it.
...
| `packages/telemetry/src/project.ts` | `posthogKey()` and `posthogHost()` read `CRM_TELEMETRY_KEY` and `CRM_TELEMETRY_HOST`; the UI host is a constant |
