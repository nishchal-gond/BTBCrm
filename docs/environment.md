...
- **Server side only**, `posthog-node` in the API and the agent. There is no
  `posthog-js` anywhere in the app. `docs/telemetry.md`.
- **The destination is yours.** `CRM_TELEMETRY_KEY` is a write-only `phc_` key for your
  own PostHog project and `CRM_TELEMETRY_HOST` its ingestion host. Without a key no client
  is constructed and nothing is sent.
